...
ActivityInstrumentationTestCase2<SettingsActivity> {
	...
	public void testActivateNotifications() {
		...
		boolean initValue = SettingsManager.getInstance()
				.notificationsEnabled();
		assertEquals("Failed to disable notifications", false, initValue);

		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_notifications_enabled)))
						.perform(ViewActions.click());
		assertEquals("Failed to enable notifications", true, SettingsManager
				.getInstance().notificationsEnabled());
	}

	public void testCanActivateEventNotifications() {
		this.enableNotifications();
		assertEquals("Cannot enable notifications", true, SettingsManager
				.getInstance().notificationsEnabled());
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_event_invitations)))
						.perform(ViewActions.click());
		assertTrue(
				"Couldn't change boolean status of notifications for event invitations",
				initValue != SettingsManager.getInstance()
				.notificationsForEventInvitations());

		boolean initValue2 = SettingsManager.getInstance()
				.notificationsForEventProximity();
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_event_proximity)))
						.perform(ViewActions.click());
		assertTrue(
				"Couldn't change boolean status of notifications for event proximity",
				initValue2 != SettingsManager.getInstance()
				.notificationsForEventProximity());
	...
	public void testCanActivateFriendRequests() {
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_friend_requests)))
						.perform(ViewActions.click());
		assertTrue(
				"Couldn't change boolean status of notifications for friend requests",
				initValue != SettingsManager.getInstance()
				.notificationsForFriendRequests());
	...
	public void testCanActivateFriendshipConfirmations() {
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_friendship_confirmations)))
						.perform(ViewActions.click());
		assertTrue(
				"Couldn't change boolean status of notifications for friendship confirmations",
				initValue != SettingsManager.getInstance()
				.notificationsForFriendshipConfirmations());
	...
	public void testCanChangeRefreshDataFrequency() {
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_refresh_frequency)))
						.perform(ViewActions.click());
		onView(ViewMatchers.withText("5 seconds")).perform(ViewActions.click());

		assertEquals("Couldn't set the frequency to 5 seconds", 5,
				SettingsManager.getInstance().getRefreshFrequency());

		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_refresh_frequency)))
						.perform(ViewActions.click());
		onView(ViewMatchers.withText("10 seconds (default)")).perform(
				ViewActions.click());

		assertEquals("Couldn't set the frequency to 10 seconds", 10,
				SettingsManager.getInstance().getRefreshFrequency());
	}
	...
	public void testCanChangeTimeToWaitBeforeHiding() {
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_last_seen_max)))
						.perform(ViewActions.click());
		onView(ViewMatchers.withText("10 minutes"))
		.perform(ViewActions.click());
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_last_seen_max)))
						.perform(ViewActions.click());
		onView(ViewMatchers.withText("30 minutes (default)")).perform(
				ViewActions.click());
	...
	public void testCanEnableVibrations() {
		...
		onData(
				Matchers.<Object> allOf(PreferenceMatchers
						.withKey("notifications_vibrate"))).perform(
								ViewActions.click());
		assertTrue(
				"Couldn't change boolean status of notifications for vibrations",
				initValue != SettingsManager.getInstance()
				.notificationsVibrate());
	}
	...
	public void testCannotActivateEventsNotificationsIfNotifDisabled() {
		...
		onView(
				ViewMatchers.withText(mContext
						.getString(ch.epfl.smartmap.R.string.pref_title_event_invitations)))
						.perform(ViewActions.click());
		assertEquals("Can enable a checkbox when its dependency is disabled",
				false, SettingsManager.getInstance()
				.notificationsForEventInvitations());
		...
		onData(
				Matchers.<Object> allOf(PreferenceMatchers
						.withKey("notifications_vibrate"))).perform(
								ViewActions.click());
		assertEquals("Can enable a checkbox when its dependency is disabled",
				false, SettingsManager.getInstance().notificationsVibrate());
	...
	public void testCanSetPublicAndPrivateEvents() {
		onData(
				Matchers.<Object> allOf(PreferenceMatchers
						.withKey("events_show_public"))).perform(
								ViewActions.click());
		...
		assertTrue("Couldn't change boolean status of public events",
				initValue != SettingsManager.getInstance().showPublicEvents());

		onData(
				Matchers.<Object> allOf(PreferenceMatchers
						.withKey("events_show_private"))).perform(
								ViewActions.click());
		...
		assertTrue("Couldn't change boolean status of private events",
				initValue2 != SettingsManager.getInstance().showPrivateEvents());
	}
	...
	public void testClickingAlwaysSharePositionChangesTheSettings() {
		...
		if (initialValue) {
			onView(
					ViewMatchers.withText(mContext
							.getString(ch.epfl.smartmap.R.string.pref_title_always_share)))
							.perform(ViewActions.click());
		}
		...
		assertTrue("intitialValue was " + initialValue + " and finalValue was "
				+ finalValue, initialValue != finalValue);
	}
