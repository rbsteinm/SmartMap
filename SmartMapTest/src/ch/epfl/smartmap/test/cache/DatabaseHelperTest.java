...
public class DatabaseHelperTest extends AndroidTestCase {
	...
	private final PublicEvent event = new PublicEvent("A new event", 1234,
			"qwertz uiop", new GregorianCalendar(), new GregorianCalendar(),
	...
	protected void setUp() throws Exception {
		super.setUp();
		dbh = DatabaseHelper.initialize(new RenamingDelegatingContext(this
				.getContext(), "test_"));
	...
	public void testAddEvent() {
		dbh.addEvent(event);
		assertTrue(dbh.getEvent(event.getID()).getCreatorName()
				.equals(event.getCreatorName())
				&& (dbh.getEvent(event.getID()).getStartDate()
						.get(GregorianCalendar.MINUTE) == event.getStartDate()
						.get(GregorianCalendar.MINUTE))
						&& (dbh.getEvent(event.getID()).getLocation().getLatitude() == event
	...
	public void testAddUser() {
		...
		assertTrue((dbh.getUser(a.getID()).getID() == a.getID())
				...
				&& dbh.getUser(a.getID()).getLocationString()
				.equals(a.getLocationString())
				&& (dbh.getUser(a.getID()).getLocation().getLongitude() == a
				.getLocation().getLongitude())
				&& (dbh.getUser(a.getID()).getLocation().getLatitude() == a
				.getLocation().getLatitude())
	...
	public void testDeleteEvent() {
		...
		assertTrue((dbh.getAllEvents().size() == 1)
				&& (dbh.getAllEvents().get(0).getID() == event2.getID()));
	}
	...
	public void testgetAllFriends() {
		...
		assertTrue((list.size() == 3)
				&& (dbh.getUser(c.getID()).getID() == c.getID()));
	}
	...
	public void testUpdateEvent() {
		...
		assertTrue(dbh.getEvent(event.getID()).getName().equals(name)
				&& (rows == 1));
	}
