...
public class NetworkEndToEndTest extends TestCase {
	...
	public void testT_createEvent() throws SmartMapClientException {
		NetworkSmartMapClient networkClient = NetworkSmartMapClient
				.getInstance();
		networkClient.createEvent(FOOTBALL_TOURNAMENT);
	}
	...
	public void testV_getPublicEvents() throws SmartMapClientException {
		...
		for (Event event : events) {
			...
			assertTrue("Unexpected end and start dates", event.getEndDate()
					.after(event.getStartDate()));
			...
			assertTrue("Unexpected position name", ((2 < event
					.getPositionName().length()) && (event.getPositionName()
							.length() <= 60)));
