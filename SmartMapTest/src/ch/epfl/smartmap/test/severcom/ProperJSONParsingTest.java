...
public class ProperJSONParsingTest extends TestCase {
	...
	private void checkEvent1(ImmutableEvent event) {
		assertEquals("creator id does not match", 3, event.getCreatorId());
		...
		assertEquals("latitude does not match", location1.getLatitude(), event.getLocation().getLatitude());
		assertEquals("Friend's longitude does not match", location1.getLongitude(),
				event.getLocation().getLongitude());
		assertEquals("Position name does not match", "Tokyo", event.getLocationString());
