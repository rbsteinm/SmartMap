...
public class ProperJSONParsingTest extends AndroidTestCase {
    ...
    public void testParseEventList() throws SmartMapParseException {
        SmartMapParser parser = new JsonSmartMapParser();
        List<EventContainer> events = parser.parseEventList(PROPER_EVENT_LIST_JSON);
    ...
    public void testParseEventListWhenEmptyList() throws SmartMapParseException {
        ...
        List<EventContainer> events = parser.parseEventList(PROPER_EVENTS_EMPTY_LIST_JSON);
    ...
    public void testParseFriends() throws SmartMapParseException {
        SmartMapParser parser = new JsonSmartMapParser();
        List<UserContainer> listFriends = parser.parseFriendList(PROPER_FRIEND_LIST_JSON, "list");
    ...
    public void testParseFriendsWhenEmptyList() throws SmartMapParseException {
        SmartMapParser parser = new JsonSmartMapParser();
        List<UserContainer> friends = parser.parseFriendList(PROPER_FRIEND_EMPTY_LIST_JSON, "list");
    ...
    public void testParsePositions() throws SmartMapParseException {
        ...
        List<UserContainer> users = parser.parsePositions(PROPER_POSITIONS_LIST_JSON);
    ...
    public void testParsePositionsWhenEmptyList() throws SmartMapParseException {
        SmartMapParser parser = new JsonSmartMapParser();
        List<UserContainer> users = parser.parsePositions(PROPER_POSITIONS_EMPTY_LIST_JSON);
    ...
    private void checkEvent1(EventContainer immutableEvent) {
        ...
        assertEquals("creator id does not match", 3, immutableEvent.getCreatorContainer().getId());
        ...
        assertEquals("latitude does not match", location1.getLatitude(), immutableEvent.getLocation()
            .getLatitude());
        assertEquals("longitude does not match", location1.getLongitude(), immutableEvent.getLocation()
            .getLongitude());
        ...
        assertTrue("Did not parse the 3 participants", immutableEvent.getParticipantIds().size() == 3);

    }

    private void checkEvent2(EventContainer event) {
        assertEquals("creator id does not match", 1, event.getCreatorContainer().getId());
        ...
        assertEquals("Friend's longitude does not match", location2.getLongitude(), event.getLocation()
            .getLongitude());
        ...
        assertTrue("Did not parse the participant", event.getParticipantIds().size() == 1);
