...
public class ProperJSONParsingTest extends TestCase {
	...
	public void testParsePositions() throws SmartMapParseException {
		SmartMapParser parser = new JsonSmartMapParser();
		Map<Long, Location> positions = parser
				.parsePositions(PROPER_POSITIONS_LIST_JSON);
		assertTrue("Did not parse the first position",
				positions.containsKey((long) 13));
		assertTrue("Did not parse the second position",
				positions.containsKey((long) 18));
		assertEquals("First location's latitude does not match",
				location1.getLatitude(), positions.get((long) 13).getLatitude());
		assertEquals("First location's longitude does not match",
				location1.getLongitude(), positions.get((long) 13)
						.getLongitude());
		assertEquals("Second location's latitude does not match",
				location2.getLatitude(), positions.get((long) 18).getLatitude());
		assertEquals("Second location's longitude does not match",
				location2.getLongitude(), positions.get((long) 18)
						.getLongitude());
