...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private final List<DatabaseListener> mListeners = new ArrayList<DatabaseListener>();
    ...
    private static final String[] USER_COLUMNS = { KEY_USER_ID, KEY_NAME,
        KEY_NUMBER, KEY_EMAIL, KEY_LONGITUDE, KEY_LATITUDE, KEY_POSNAME,
        KEY_LASTSEEN, KEY_VISIBLE };

    // Columns for the Filter table
    private static final String[] FILTER_COLUMNS = { KEY_ID, KEY_NAME };

    // Columns for the Filter/User table
    private static final String[] FILTER_USER_COLUMNS = { KEY_ID,
        KEY_FILTER_ID, KEY_USER_ID };

    // Columns for the Event table
    private static final String[] EVENT_COLUMNS = { KEY_ID, KEY_NAME,
        KEY_EVTDESC, KEY_USER_ID, KEY_CREATOR_NAME, KEY_LONGITUDE,
        KEY_LATITUDE, KEY_POSNAME, KEY_DATE, KEY_ENDDATE };
    ...
    private static final String CREATE_TABLE_FILTER_USER = "CREATE TABLE IF NOT EXISTS "
        ...
        + KEY_ID
        + " INTEGER PRIMARY KEY,"
        + KEY_FILTER_ID + " INTEGER," + KEY_USER_ID + " INTEGER" + ")";
    ...
    public void addEvent(Event event) throws IllegalArgumentException {
        ...
        Cursor cursor = mDatabase.query(TABLE_EVENT, EVENT_COLUMNS, KEY_ID
            + " = ?", new String[] { String.valueOf(event.getID()) }, null,
            null, null, null);
    ...
    public List<Event> getAllEvents() {
        ...
        if (cursor.moveToFirst()) {
            do {
                events.add(this.getEvent(cursor.getLong(cursor
    ...
    public List<UserList> getAllFilters() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                filters.add(this.getFilter(cursor.getLong(cursor
                    .getColumnIndex(KEY_ID))));
            } while (cursor.moveToNext());
        }
    ...
    public LongSparseArray<User> getAllUsers() {
        ...
        if (cursor.moveToFirst()) {
            do {
                friend = new Friend(cursor.getLong(cursor
                    .getColumnIndex(KEY_USER_ID)), cursor.getString(cursor
                    .getColumnIndex(KEY_NAME)));
                friend.setNumber(cursor.getString(cursor
                    .getColumnIndex(KEY_NUMBER)));
                friend.setEmail(cursor.getString(cursor
                    .getColumnIndex(KEY_EMAIL)));
                friend.setLongitude(cursor.getDouble(cursor
                    .getColumnIndex(KEY_LONGITUDE)));
                friend.setLatitude(cursor.getDouble(cursor
                    .getColumnIndex(KEY_LATITUDE)));
                friend.setPositionName(cursor.getString(cursor
                    .getColumnIndex(KEY_POSNAME)));
                GregorianCalendar cal = new GregorianCalendar();
                cal.setTimeInMillis(cursor.getLong(cursor
                    .getColumnIndex(KEY_LASTSEEN)));
                friend.setLastSeen(cal);
                friend.setVisible(cursor.getInt(cursor
                    .getColumnIndex(KEY_VISIBLE)) == 1); // int to boolean
    ...
    public Event getEvent(long id) {
        ...
        startDate.setTimeInMillis(cursor.getLong(cursor
            .getColumnIndex(KEY_DATE)));
        endDate.setTimeInMillis(cursor.getLong(cursor
            .getColumnIndex(KEY_ENDDATE)));
        ...
        UserEvent event = new UserEvent(cursor.getString(cursor
            .getColumnIndex(KEY_NAME)), cursor.getLong(cursor
            .getColumnIndex(KEY_USER_ID)), cursor.getString(cursor
            .getColumnIndex(KEY_CREATOR_NAME)), startDate, endDate, loc);

        event.setID(id);
        event.setDescription(cursor.getString(cursor
            .getColumnIndex(KEY_EVTDESC)));
        event.setPositionName(cursor.getString(cursor
            .getColumnIndex(KEY_POSNAME)));
    ...
    public UserList getFilter(long id) {
        ...
        FriendList filter = new FriendList(cursor.getString(cursor
            .getColumnIndex(KEY_NAME)));
        filter.setID(id);
        ...
        if (cursor.moveToFirst()) {
            do {
                filter.addUser(cursor.getLong(cursor
    ...
    public User getUser(long id) {
        ...
        Friend friend = new Friend(cursor.getLong(cursor
            .getColumnIndex(KEY_USER_ID)), cursor.getString(cursor
            .getColumnIndex(KEY_NAME)));
        ...
        friend.setLongitude(cursor.getDouble(cursor
            .getColumnIndex(KEY_LONGITUDE)));
        friend
            .setLatitude(cursor.getDouble(cursor.getColumnIndex(KEY_LATITUDE)));
        friend.setPositionName(cursor.getString(cursor
            .getColumnIndex(KEY_POSNAME)));
        GregorianCalendar cal = new GregorianCalendar();
        ...
        friend.setLastSeen(cal);
        friend
            .setVisible(cursor.getInt(cursor.getColumnIndex(KEY_VISIBLE)) == 1); // int
                                                                                 // to
                                                                                 // boolean
    ...
    public int refreshFriendsPos() {
        ...
        try {
            Map<Long, Location> locations = NetworkSmartMapClient.getInstance()
                .listFriendsPos();
            LongSparseArray<User> friends = this.getAllUsers();
            User friend;
            for (int i = 0; i < friends.size(); i++) {
