...
public class Notifications {
	...
	public static void acceptedFriendNotification(Context context, ImmutableUser user) {
		...
		showFriendInfoIntent.putExtra("FRIEND", user.getId());
		PendingIntent pFriendIntent = PendingIntent.getActivity(context, 0, showFriendInfoIntent,
		...
		events[0] = new String(user.getName() + " "
	 ...
	 *            the Event
	 */
	public static void newEventNotification(final Context context, final Event event) {

		// Get ID and the number of ongoing Event notifications
		notificationID++;

		CachedSearchEngine.getInstance().findUserById(event.getCreatorId(),
		    new SearchRequestCallback<User>() {

			    @Override
			    public void onNetworkError() {
				    Log.e(TAG, "Error with network while looking for event's creator");
			    }

			    @Override
			    public void onNotFound() {

				    Log.e(TAG, "Couldn't find creator of event");
			    }
			    ...
			    public void onResult(User result) {
				    ...
				    showEventInfoIntent.putExtra("EVENT", event.getId());
				    PendingIntent pEventIntent = PendingIntent.getActivity(context, 0, showEventInfoIntent,
				        PendingIntent.FLAG_UPDATE_CURRENT);
