...
public class EventsListItemAdapter extends ArrayAdapter<Event> {
	...
	private final static String TAG = EventsListItemAdapter.class
	    .getSimpleName();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		convertView = inflater.inflate(R.layout.gui_event_list_item, parent,
		    false);
		...
		TextView startDateText = (TextView) convertView
		    .findViewById(R.id.eventStartDate);
		TextView endDateText = (TextView) convertView
		    .findViewById(R.id.eventEndDate);
		...
		double distanceMeEvent = ShowEventsActivity.distance(
		    mMyLocation.getLatitude(), mMyLocation.getLongitude(),
		    mItemsArrayList.get(position).getLocation().getLatitude(),
		    mItemsArrayList.get(position).getLocation().getLongitude());
		distanceMeEvent = Math.floor(distanceMeEvent * HUNDRED_PERCENT)
		    / HUNDRED_PERCENT;
	 ...
	 * @author SpicyCH
	 */
	public static String setTextFromDate(GregorianCalendar date1,
	    GregorianCalendar date2, String s) {
		...
		GregorianCalendar midnight = new GregorianCalendar(
		    ...
		    now.get(GregorianCalendar.DAY_OF_MONTH), MIDNIGHT_HOUR,
		    MIDNIGHT_MINUTES);
		...
		String startHourOfDayString = TimePickerFragment.formatForClock(date1
		    .get(GregorianCalendar.HOUR_OF_DAY));
		String startMinuteString = TimePickerFragment.formatForClock(date1
		    .get(GregorianCalendar.MINUTE));

		String endHourOfDayString = TimePickerFragment.formatForClock(date2
		    .get(GregorianCalendar.HOUR_OF_DAY));
		String endMinuteString = TimePickerFragment.formatForClock(date2
		    .get(GregorianCalendar.MINUTE));
		...
		if (date1.before(midnight) && date2.before(midnight)) {
			...
			if (s.equals("start")) {
				...
				dateTextContent = "from " + startHourOfDayString + ":"
				    + startMinuteString + " to " + endHourOfDayString + ":"
				    + endMinuteString;
			}
		} else if (date1.before(tomorrowMidnight)
		    && date2.before(tomorrowMidnight)) {
			// ends and starts the same day
			if (s.equals("start")) {
			...
			if (date1.before(midnight) && s.equals("start")) {
				dateTextContent = "Today at " + startHourOfDayString + ":"
				    + startMinuteString;
			} else if (date2.before(midnight) && s.equals("end")) {
				dateTextContent = "Ends today at " + endHourOfDayString + ":"
				    + endMinuteString;
			} else if (date1.before(tomorrowMidnight) && s.equals("start")) {
				dateTextContent = "Tomorrow at " + startHourOfDayString + ":"
				    + startMinuteString;
			} else if (date2.before(tomorrowMidnight) && s.equals("end")) {
				dateTextContent = "Ends tomorrow at " + startHourOfDayString
				    + ":" + startMinuteString;
			} else {
				if (s.equals("start")) {
					dateTextContent = "Starts: "
					    ...
					    + date1.get(GregorianCalendar.YEAR) + " at "
					...
					dateTextContent = "Ends: "
					    ...
					    + date2.get(GregorianCalendar.YEAR) + " at "
