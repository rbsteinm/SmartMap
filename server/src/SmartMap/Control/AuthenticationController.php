...
class AuthenticationController {
    ...
    public function authenticate(Request $request) {
        ...
        try {
            ...
            return new JsonResponse (array('status' => "Error", 'message' => 'Invalid facebook session'));
        } catch (\Exception $ex) {
            // Graph API returned info, but it may mismatch the current app or have expired.
            return new JsonResponse (array('status' => "Error", 'message' => 'Mismatching or expired facebook data.'));
        }
        ...
        if ($name != $user_profile->getName() OR $facebookId != $user_profile->getId()) {
            ...
            return new JsonResponse (array('status' => "Error", 'message' => 'Error during facebook request.'));
        ...
        if ($session == null) {
            throw ControlException('Session is null. Did you send session cookie ?');
        }
